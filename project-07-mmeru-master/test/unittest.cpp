...
class SelfDestructNotePointerFactory {
private:
  std::vector<Note *> notes_;
  bool clean_up_;

public:
  SelfDestructNotePointerFactory(bool clean_up) : clean_up_(clean_up) {}
  SelfDestructNotePointerFactory() : SelfDestructNotePointerFactory(true) {}
  SelfDestructNote *build() {
    SelfDestructNote *ptr_note = new SelfDestructNote;
    notes_.push_back(ptr_note);
    return ptr_note;
  }

  SelfDestructNote *build(const std::string &title, const std::string &body,
                          const std::string &password) {
    SelfDestructNote *ptr_note = new SelfDestructNote(title, body, false);
    return ptr_note;
  }
  ~SelfDestructNotePointerFactory() {
    if (clean_up_) {
      for (int i = 0; i < notes_.size(); i++) {
        if (notes_[i] != nullptr) {
          delete notes_[i];
...
TEST(Milestone6, CreateSelfDestructNoteChoice) {
  std::string unittest_output =
      "Welcome to TuffyNotes!\n\n[C] Create a note\n[N] Create an encrypted "
      "note\n[X] Create a self-destruct note\n[L] List notes\n[V] View "
      "note\n[S] Save notes\n[O] Load notes\n[E] Exit\nChoice: \nPlease enter "
...
TEST(Milestone7, ViewSelfDestructNoteChoice) {
  std::string unittest_output =
      "Welcome to TuffyNotes!\n\n[C] Create a note\n[N] Create an encrypted "
      "note\n[X] Create a self-destruct note\n[L] List notes\n[V] View "
      "note\n[S] Save notes\n[O] Load notes\n[E] Exit\n"
      ...
      "you for using TuffyNotes!\n";
  ASSERT_DURATION_LE(3, {
    ASSERT_EXECIO_EQ("tuffynotes", "x\nX\nX\nv\n1\ne\n", unittest_output);
  });
}

TEST(Milestone7, ViewSelfDestructNoteChoiceSecondView) {
  std::string unittest_output =
      "Welcome to TuffyNotes!\n\n[C] Create a note\n[N] Create an encrypted "
      "note\n[X] Create a self-destruct note\n[L] List notes\n[V] View "
      "note\n[S] Save notes\n[O] Load notes\n[E] Exit\n"
...
TEST_F(FileIO, Milestone1_4_6_ChoiceLoopLowercase) {
  ...
  char saved_first_note_type = 'j', first_note_type = 'j';
  bool saved_first_note_read = false, first_note_read = false;
  char choice = ' ';
  do {
    choice = choices[rand() % 8];
        ...
        if (first_note_type == 'c') {
          ...
          unittest_output +=
              note_list + "\nPlease input note index: \nPlease enter password "
                          "to view note: [t]\nb\n\n";
          input += "1\\np\\n";
        } else if (first_note_type == 'x') {
          if (first_note_read == false) {
            unittest_output +=
                note_list + "\nPlease input note index: \n[t]\nb\n\nThe note "
                            "will self-destruct in 3.. 2.. 1..\n\n";
            input += "1\\n";
            first_note_read = true;
          } else {
            unittest_output += note_list +
                               "\nPlease input note index: \nThe note has "
    ...
    case 's':
      ...
      saved_first_note_read = first_note_read;
      break;
    case 'o':
      ...
      first_note_read = saved_first_note_read;
      break;
    case 'e':
      unittest_output += "\nThank you for using TuffyNotes!\n";
      break;
...
TEST(Milestone7, SelfDestructNoteInheritance) {
  bool self_destruct_note_inherits_from_note =
      std::is_base_of<Note, SelfDestructNote>::value;
  ASSERT_TRUE(self_destruct_note_inherits_from_note);
}

TEST(Milestone7, SelfDestructNoteClass) {
  SelfDestructNote temp;
  temp.set_title("title");
  temp.set_body("body");
  temp.title();
  temp.body();
...
TEST(Milestone7, SelfDestructNoteTitleGetSet) {
  SelfDestructNote your_selfdestructnote_object;
  std::string unittest_title = generate_string(10);
  your_selfdestructnote_object.set_title(unittest_title);
  ASSERT_EQ(your_selfdestructnote_object.title(), unittest_title);
}

TEST(Milestone7, SelfDestructNoteBodyGetSet) {
  SelfDestructNote your_selfdestructnote_object;
  std::string unittest_body = generate_string(10);
  your_selfdestructnote_object.set_body(unittest_body);
  ASSERT_EQ(your_selfdestructnote_object.body(), unittest_body);
}

TEST(Milestone7, SelfDestructNoteDefaultConstructor) {
  EncryptedNote your_selfdestructnote_object;
  ASSERT_EQ(your_selfdestructnote_object.title(), "Untitled");
  ASSERT_EQ(your_selfdestructnote_object.body(), "");
}

TEST(Milestone7, SelfDestructNoteNonDefaultConstructor) {
  std::string unittest_title, unittest_body;
  unittest_title = generate_string(10);
  unittest_body = generate_string(10);
  SelfDestructNote your_encryptednote_object(unittest_title, unittest_body,
                                             false);
  ASSERT_EQ(your_encryptednote_object.title(), unittest_title);
  ASSERT_EQ(your_encryptednote_object.body(), unittest_body);
}

TEST(Milestone7, SelfDestructNoteFirstView) {
  std::string unittest_title, unittest_body, unittest_display;
  unittest_title = generate_string(10);
  unittest_body = generate_string(10);
  unittest_display = "[" + unittest_title + "]\n" + unittest_body +
                     "\n\nThe note will self-destruct in 3.. 2.. 1..\n\n";
  SelfDestructNote your_selfdestructnote_object(unittest_title, unittest_body,
                                                false);
  ASSERT_DURATION_LE(3, ASSERT_SIO_EQ("", unittest_display, {
                       your_selfdestructnote_object.display();
                     }));
}

TEST(Milestone7, SelfDestructNoteSecondView) {
  std::string unittest_title, unittest_body, unittest_display;
  unittest_title = generate_string(10);
  unittest_body = generate_string(10);
  unittest_display = "The note has already been destroyed.\n\n";
  SelfDestructNote your_selfdestructnote_object(unittest_title, unittest_body,
                                                true);
  ASSERT_DURATION_LE(3, ASSERT_SIO_EQ("", unittest_display, {
                       your_selfdestructnote_object.display();
...
TEST(Milestone7, SelfDestructNoteSerialize) {
  std::string unittest_title, unittest_body, unittest_displays;
  unittest_title = generate_string(10);
  unittest_body = generate_string(10);
  SelfDestructNote your_selfdestructnote_object(unittest_title, unittest_body,
                                                false);
  ASSERT_EQ(your_selfdestructnote_object.serialize(),
            "[SDNote]\n" + unittest_title + "\n" + unittest_body + "\n" +
                std::to_string(false) + "\n")
      << "Where unittest_title = " << unittest_title
      << " unittest_body = " << unittest_body;
...
TEST(Milestone7, CreateSelfDestructNote) {
  SelfDestructNote *your_selfdestructnote_object;
  std::string unittest_title = generate_string(10);
  std::string unittest_body = generate_string(10);
  std::string input = unittest_title + "\n" + unittest_body + "\n";
  ...
  // create_note returns a pointer. should we manage this?
  ASSERT_SIO_EQ(input, unittest_output, {
    your_selfdestructnote_object = create_self_destruct_note();
  });
  ASSERT_EQ(your_selfdestructnote_object->title(), unittest_title);
  ASSERT_EQ(your_selfdestructnote_object->body(), unittest_body);
}
...
TEST(Milestone6, ViewEncryptedNoteIncorrectPassword) {
  ...
  ASSERT_DURATION_LE(3, {
    ASSERT_SIO_EQ(
        "2\\nx",
        "\nNotes\n1. Group mates list\n2. Buy groceries\n\nPlease input note "
        "index: \nPlease enter password to view note: Sorry, you do not have "
        "the permission to view note.\n\n",
        { unittest_notebook.view(); });
  });
}
...
TEST_F(FileIO, Milestone5_6_7_LoadSelfDestructNotes) {
  SelfDestructNotePointerFactory nf(false);
  int unittest_notes_size = rand() % 10 + 1;
  Notebook unittest_notebook;
  std::ofstream the_file;
  the_file.open(UNITEST_FILENAME);
  the_file << unittest_notes_size << "\n";
  for (int i = 0; i < unittest_notes_size; i++) {
    SelfDestructNote *temp = nf.build();
    the_file << "[Note]\n";
    std::string temp_string = generate_string(10);
    temp->set_title(temp_string);
    the_file << temp_string << "\n";
...
TEST_F(FileIO, Milestone7_SaveSelfDestructNoteChoice) {
  std::string unittest_output =
      "Welcome to TuffyNotes!\n\n[C] Create a note\n[N] Create an encrypted "
      "note\n[X] Create a self-destruct note\n[L] List notes\n[V] View "
      "note\n[S] Save notes\n[O] Load notes\n[E] Exit\n"
